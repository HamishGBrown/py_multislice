...
class orbital:
    def __init__(self,Z : int,config : str,n : int,l : int,
                 epsilon = 0):
        ...
        self.n  = n
        self.l  = l
        if(self.n==0): self.epsilon = epsilon
        ...
        if(n==0): 
            configstring = ATOMICSYMBOL[Z]+'ex'
        else:
            configstring = ATOMICSYMBOL[Z]+'bound'
        # Set up configuration
        Config(configstring, config)
        # Optimize atomic energy levels
        ConfigEnergy(0)
        # Optimize radial wave functions
        OptimizeRadial(configstring)
        ...
        if (n>0):
            ...
            self.title = '{0} {1}{2}'.format(ATOMICSYMBOL[Z],n,angmom)
        else:
            #Continuum wave function case
            #Title in the format "Ag e = 10 eV l'=2" etc..
            self.title = "{0} \\varepsilon = {1} l' = {2}".format(ATOMICSYMBOL[Z],
                                                              epsilon,l)
        ...
        WaveFuncTable('orbital.txt',n,kappa,epsilon)
        ...
        #
        if (self.n==0):
            self.__amplitude = interp1d(table[self.ilast-1:,1],
                                        table[self.ilast-1:,2],fill_value=0)
            self.__phase     = interp1d(table[self.ilast-1:,1],
                                        table[self.ilast-1:,3],fill_value=0)

        # For bound wave functions we simply interpolate the
        # tabulated values of a0 the wavefunction
        # *2
        self.__wfn = interp1d(table[:self.ilast,1],
                              table[:self.ilast,4],
    ...
    def wfn(self,r):
        ...
        if(self.n>0): return wvfn
        
        # For a continuum wave function inbetween region I and II
        # interpolate between the regions
        mask = np.logical_and(r_>=self.r[self.ilast-1] , r_<=self.r[self.ilast+1])
        if(np.any(mask)):
        ...
        wvfn[mask] =  self.__amplitude(r_[mask])*\
                        np.sin(self.__phase(r_[mask]))
        if(is_arr): return wvfn
        else: return wvfn[0]
...
def transition_potential(orb1,orb2,gridshape,gridsize,ml,mlprime,keV,
    ...
    qgrid  = [np.fft.fftfreq(gridshape[1])*qspace[0], \
              np.fft.fftfreq(gridshape[0])*qspace[1]]
    
    # Transverse momentum transfer
    qt = np.sqrt(qgrid[0][:,np.newaxis]**2 +\
                 qgrid[1][np.newaxis,:]**2)
    ...
    if (np.abs(ml)     > l): return Hn0
    if (np.abs(mlprime)>lprime): return Hn0
    ...
    def ovlap(q,lprimeprime):
        ...
        rmax = 0
        if(orb1.n>0): rmax = orb1.r[orb1.ilast-1]
        if(orb2.n>0): rmax = max(rmax,orb2.r[orb2.ilast-1])
        
        # The following ensures that q can be passed as a single value or
        # as an array
        is_arr = isinstance(q,np.ndarray)
        if is_arr: q_ = q
        else: q_ = np.asarray([q])
        ...
        for iQ,Q in enumerate(np.ravel(q_)):
            ...
            overlap_kernel = lambda x: orb1.wfn(x)*spherical_jn(lprimeprime,2*np.pi*Q*a0*x) \
                                    *orb2.wfn(x)
    ...
    if(lprimeprimes.shape[0]<1): return None
    ...
    for lprimeprime in lprimeprimes:
        ...
        for mlprimeprime in mlprimeprimes:
            ...
            if(np.abs(prefactor2)>1e-12):
                ...
                if jq is None: jq = interp1d(qmesh,ovlap(qmesh,lprimeprime))\
                                                               (qabs[qmask])
    ...
    #in real space
    return qe/4/np.pi**2/eps0*sigma*np.fft.ifft2(Hn0/qabs**2)*np.sqrt(2)
...
if __name__ == "__main__":
    ...
    LaFree = orbital(57,'1s2 2s2 2p6 3s2 3p6 4s2 3d10 4p6 5s2 4d9 5p6 6s2 5d1',0,3,epsilon=1)
    ...
    for rdims in ([40,40],[50,50])[:1]:
        ...
        Hn0 = np.fft.fftshift(transition_potential(LaN45,LaFree,grid,rdims,0,0,keV))
        ...
        ax[1,0].plot(np.linspace(-rdims[0]/2,rdims[0]/2,num=grid[0]),np.mean(np.abs(Hn0)**2,axis=0))
    ...
    for iqnum,qnums in enumerate([(0,0),(1,0),(1,1)]):
        ...
        ax.plot_surface(grid[:,np.newaxis],grid[np.newaxis,:],funcs[iqnum](Hn0))#,
                        # facecolors = colors)
        ax.set_xlabel(r'x')
        ax.set_ylabel(r'y')

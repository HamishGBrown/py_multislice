...
def torch_c_exp(angle):
    ...
    if(angle.size()[-1]!=2):
        ...
        result = torch.zeros(*angle.shape,2,dtype=angle.dtype,
                                           device=angle.device)
...
def torch_plot(array):
    ...
    if (array.size()[-1]==2):
        ...
        axes[0].imshow(np.angle(array[...,0].cpu().numpy()+
                          1j*array[...,1].cpu().numpy()))
        axes[0].set_title('Phase')
        axes[1].imshow(np.square(array[...,0].cpu().numpy())+
                       np.square(array[...,1].cpu().numpy()))
        axes[1].set_title('Amplitude')
        axes[2].imshow(colorize(array[...,0].cpu().numpy()+
                             1j*array[...,1].cpu().numpy()))
...
def modulus_square(r): return torch.sum(amplitude(r))

def amplitude(r): return r[...,0]*r[...,0]+r[...,1]*r[...,1]

def roll_n(X, axis, n):
    f_idx = tuple(slice(None, None, None) if i != axis% X.dim() 
             else slice(0, n, None)      for i in range(X.dim()))
    b_idx = tuple(slice(None, None, None) if i != axis% X.dim() 
...
def batch_fftshift2d_real(x,axes = None):
    ...
    if(axes is None): axes = [x for x in range(len(x.size()))]
...
def batch_fftshift2d(x,axes = None):
    ...
    #an FFT shift to all axes
    if(axes is None): axes = [x for x in range(len(real.size()))]
...
def scatter_add_patches(input: torch.Tensor, out: torch.Tensor, axes, positions, patch_size,index=None) -> torch.Tensor:
    """
    ...
    """
    if(index is None): index = get_scatter_gather_indices(positions,patch_size,out,axes)
...
def get_scatter_gather_indices(r,s,input,axes,edge_behaviour='periodic'):
    ...
    start = -s[0]//2 +s[0]%2
    stop  =  s[0]//2 +s[0]%2
    index0 = torch.arange(start,stop,device=input.device, dtype=torch.long)\
    .view(s[0], 1).expand(s[0], s[1])
    start = -s[1]//2 +s[1]%2
    stop  =  s[1]//2 +s[1]%2
    index1 = torch.arange(start,stop,device=input.device, dtype=torch.long)\
    .view(1, s[1]).expand(s[0], s[1])
    ...
    if(edge_behaviour=='periodic'):
        index =  input.stride(axes[0]) * (torch.remainder(index0 + r[:, 0].view(K, 1, 1),
                 input.shape[axes[0]]))\
               + input.stride(axes[1]) * (torch.remainder(index1 + r[:, 1].view(K, 1, 1),
                 input.shape[axes[1]]))
    else:
        index =  input.stride(axes[0]) * (torch.clamp(index0 + r[:, 0].view(K, 1, 1),
            0,input.shape[axes[0]]))\
           + input.stride(axes[1]) * (torch.clamp(index1 + r[:, 1].view(K, 1, 1),
            0,input.shape[axes[1]]))
    ...
    # array
    higher_dim_offsets = torch.arange(input.stride(axes[1]), device=input.device) \
                                .view(1, 1, 1, input.stride(axes[1]))
    ...
    # in the input axes
    dim0size = torch.prod(torch.Tensor([input.shape[:axes[0]]])).int().item() \
    if axes[0] > 0 else 1
...
def gather_patches(input, axes, positions, patch_size,index = None) -> torch.Tensor:
    ...
    if(index is None): index = get_scatter_gather_indices(r,s,input,axes).contiguous().view(-1)
...
def torch_bin(array,factor=2):
    ...
    result = torch.zeros(*array.size()[:2],biny,binx,device=array.device,dtype = array.dtype)
...
def correlate(arr1,arr2):
    ...
    if complex_input :
        ...
        a_ = torch.cat([x.view(*arr1.size(),1) for x in[arr1,torch.zeros_like(arr1)]],-1)
        b_ = torch.cat([x.view(*arr2.size(),1) for x in[arr2,torch.zeros_like(arr2)]],-1)
...
def fourier_shift_array_1d(y,posn,dtype=torch.float,
...
def fourier_shift_array(size,posn,dtype=torch.float,
                       device = torch.device('cpu')):
